...
INSTANCE_URI_TO_LOCAL_ROUTE = {
    "http://linked.data.gov.au/dataset/asgs2016/meshblock/": "/meshblock/",
    "http://linked.data.gov.au/dataset/asgs2016/statisticalarealevel1/": "/statisticalarealevel1/",
    "http://linked.data.gov.au/dataset/asgs2016/statisticalarealevel2/": "/statisticalarealevel2/",
    "http://linked.data.gov.au/dataset/asgs2016/statisticalarealevel3/": "/statisticalarealevel3/",
        ...
        with open("{}/{}_p{}_s{}.nt".format(OUTPUT_DIRECTORY, reg_uri_to_filename(reg_uri), str(worker_index+1), str(iig+1)),
            ...
            for inst in instance_s_group:
                ...
                if m is None:
                    ...
                    endpoint_rule, endpoint_func, m = find_matching_app_route(stripped_url)
                if local_instance_url.startswith('http://') or local_instance_url.startswith('https://'):
                    dummy_request_uri = local_instance_url
                elif '?' in local_instance_url:
                ...
                if isinstance(resp, pyldapi.Renderer):
                    ...
                    resp = resp.render()
                if hasattr(resp, 'status_code') and (resp.status_code == 302 or resp.status_code == 301):
                    loc = resp.headers.get('Location', None)
                    if loc is None:
                        err("302|301 redirect, but cannot find the new location.")
...
def find_matching_app_route(url):
    for rule in app.url_map.iter_rules():
        m = rule.match("|"+url)
        if m is not None:
            endpoint_func = app.view_functions[rule.endpoint]
    ...
    return (endpoint_rule, endpoint_func, m)
...
def harvest_register(reg_uri):
    ...
    endpoint_rule, endpoint_func, m = find_matching_app_route(first_local_instance_url)
